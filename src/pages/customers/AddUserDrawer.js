...
function SidebarAddCourier({ open, toggle, setChange, user, edit, showUser }) {
  ...
  useEffect(() => {
    ...
    if (user) {
      setSelectedSenderOstan(ostan.find(element => element.name === user.ostan)?.id)
    }
  }, [hasDiscount])
  ...
  const defaultValues = user
    ? {
        ...
        chapter_code: user.chapter_code
      }
    : emptyForm
              ...
              render={({ field: { onChange, onBlur } }) => (
                <>
                  ...
                  <Select
                    ...
                    disabled={showUser}
                    defaultValue={user ? user.marketer : 0}
                  >
