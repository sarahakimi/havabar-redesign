...
function SidebarAddCourier({ open, toggle, setChange, user, edit, showUser, roles }) {
  ...
  const defaultValues = user
    ? {
        ...
        password: '******',
        roles: roles.filter(item => user[item.value] === true && item)
      }
    : emptyForm
  ...
  useEffect(() => {
    ...
    user &&
      setValue(
        'roles',
        roles.filter(item => user[item.value] === true && item)
      )
  }, [])
  ...
  const onSubmit = data => {
    ...
    const selectedRoles = data.roles.reduce((obj, item) => Object.assign(obj, { [item.value]: true }), {})

    const allRoles = roles.reduce((obj, item) => Object.assign(obj, { [item.value]: false }), {})
    ...
    if (edit) {
      ...
      toast.promise(
        editUser(user.natural_code, apiData).then(() => {
          setChange(true)
          handleClose()
        }),
        {
              ...
              render={({ field: { onChange, onBlur } }) => (
                <>
                  ...
                  <Select
                    ...
                    id='demo-multiple-name'
                    multiple
                    defaultValue={user ? roles.filter(item => user[item.value] === true && item) : []}
                    onChange={onChange}
