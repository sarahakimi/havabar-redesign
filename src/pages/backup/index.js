...
function ACLPage() {
  ...
  const downloadApi = () =>
    toast.promise(
      fetchData({ sort_by: sortModel.sort_by, search: sortModel.search }).then(response => {
        setDownloadData(
  ...
  const getBackupbyId = row => {
    toast.promise(
      downloadBackup(row.id).then(response => {
  ...
  const setBackupById = id =>
    toast.promise(applyBackupById(id), {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Button onClick={() => setBackupById(row.id)}>اعمال فایل</Button>
        </Box>
      )
    },
    {
      ...
      renderCell: ({ row }) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Button onClick={() => getBackupbyId(row)}>دانلود</Button>
        </Box>
      )
    }
  ]
  ...
  return (
    <Grid container spacing={6}>
      <Grid item xs={12}>
        <Card>
          <CardContent>
            <TableHeader data={downloadData} api={downloadApi} headers={headers} name='پشتیبان گیری' noAdd noExport>
              <>
